    ...
    def request(self, data, session=None):
        ...
        while True:
            ...
            try:
                ...
                LOG.error(error)
                self.client.callback("ServerUnreachable", {'ServerId': self.client.auth.server_id})
                ...
                if r.status_code == 401:
                    ...
                    if 'X-Application-Error-Code' in r.headers:
                        self.client.callback("AccessRestricted", {'ServerId': self.client.auth.server_id})

                        raise HTTPException("AccessRestricted", error)
                    else:
                        self.client.callback("Unauthorized", {'ServerId': self.client.auth.server_id})
